...
from model import Graph2HybridMetric

def create_graph2hybridmetric_gnn_model(hidden_state_size: int = 64, dropout_rate: float = 0.1):
...
def run(arguments):
    ...
    if restore_path:
        ...
        model, nn = Graph2HybridMetric.restore_model(Path(restore_path), device="cpu")
    elif arguments["--aml"] and model_path.exists():
        initialize_metadata = False
        model, nn = Graph2HybridMetric.restore_model(model_path, device="cpu")
    else:
        nn = None
        model = create_graph2hybridmetric_gnn_model()
